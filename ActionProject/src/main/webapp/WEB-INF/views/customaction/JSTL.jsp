...
<body>
	...
	<p>
		...
		[ 표현법 ]
		&lt;@ taglib prefix="접두어"" uri="파일url">
	</p>

	<hr>

	<h4>JSTL Core Library(오늘의 주인공이였던 것)</h4>

	<p>
		변수와 조건문, 반복문 등의 로직과 관련된 태그들을 제공
	</p>

	<h5>변수((사실변수아님)속성선언하기 == Attribute)</h5>

	<pre>
		아까 배운 Scope들에 새로운 속성 추가하는 방법입니다.

		[ 표현법 ]
		&lt; c:set var="키값" value="리터럴값" scope="스코프영역지정(생략가능)" />
		- Scope에 새로운 Attribute를 추가할 수 있는 태그
		- 더 나아가서 어떤 Scope에 추가 할건지도 지정 가능(생략 시 pageScope에 담김)
	...
	<c:set var="result" scope="request"></c:set>
	...
	<pre>
		* 속성 출력(&lt;c:out value="츌력할 값" default="기본값" escapeXml="t/f" />)
		- 데이터를 출력하려고 할 때 사용하는 태그
		- default : 기본 값, value속성에 출력하고자 하는 값이 없을 경우 대체해서 출력할
				 	내용물을 쓸 수 있음(생략 가능)
		- escapeXml : HTML태그를 인식 시킬수도 있음.
	</pre>
	...
	out태크를 써볼까? : <c:out value="${ requestScope.result }" default="값이 없어용.." />
	...
	<pre>
		...
		choose안에는 when/otherwise말고는 작성 불가능
		(when / otherwise 안에 들어가는건 가능은 함)
	</pre>
	...
	<pre>
		...
		var로 선언된 제어변수를 사용할 때는 반드시 EL구문으로 접근 해야함!
	</pre>
