...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * => key-value세트로 담겨잇음(name속성값==value속성값)
		...
		// "셀 수 없음"
		// "홍길동" / ""(텍스트 상자가 비어있을 경우 빈 문자열) / null
		...
		// 보통의 흐름 : Controller에서 Service의 모소드를 호출하면서 값을 전달
		...
		pw.println("#gae{color : orangered}");
		...
		pw.println("</head>");
			pw.println("<body>");
				...
				if(gender == null || "선택안함".equals(gender)) {
					pw.println("성별 선택을 안했습니다.<br>");
				} else if(gender.equals("남")) {
					pw.println("<span id='gender'>남자</span>입니다.<br>");
				} else {
					pw.println("<span id='gender'>여자</span>입니다. <br>");
				}
